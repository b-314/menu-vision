...
async def process_menu_text(request: MenuTextRequest):
    ...
    try:
        ...
        structured_menu = await parser.parse()  # returns list of dicts with categories and items
...
def explain_nutrient(nutrient: str):
    """
    Text-to-speech endpoint for nutrient explanations
    """
    ...
    if not ELEVEN_API_KEY:
        return JSONResponse(
            {"error": "ElevenLabs API key not configured", "text": text}, 
            status_code=500
        )
    ...
    try:
        ...
        if resp.status_code != 200:
            return JSONResponse(
                {"error": "TTS failed", "text": text, "details": resp.text}, 
                status_code=500
            )
        ...
        return JSONResponse(
            {"error": "TTS request failed", "text": text, "details": str(e)}, 
            status_code=500
        )